...
def get_or_error(
    config: Section, option: str, error_msg: str, default: Optional[Any] = None
):
...
class Preferences:
    ...
    async def _init(
        ...
        uid = os.geteuid()
        log_file = (
            log_file or "/var/log/cloudflare-ddns.log"
            if uid == 0
            else f"{home}/log/cloudflare-ddns.log"
        )
        pid_file = (
            pid_file or "/run/cloudflare-ddns.pid"
            if uid == 0
        ...
        self.domain = get_or_error(
            cloudflare, "domain", error_base % "Domain", default=domain
        )
        self.name = get_or_error(
            cloudflare, "name", error_base % "Record (name)", default=name
        )
        self.type = get_or_error(
            cloudflare, "type", error_base % "Record (type)", default=rtype
        )
        self.ttl = int(
            get_or_error(
                cloudflare, "ttl", error_base % "TTL (Time To Live)", default=ttl
            )
        )
        self.frequency = int(
            get_or_error(
        ...
        self.key = get_or_error(
            cloudflare, "api-key", error_base % "API Key", default=key
        )
        self.mail = get_or_error(cloudflare, "mail", error_base % "Mail", default=mail)
        self.use_proxy = bool(
            get_or_error(
                cloudflare, "use-proxy", error_base % "Proxied", default=str(use_proxy)
            )
        )

        self.logging_file = get_or_error(
            log, "file", error_base % "Log file", default=log_file
        )
        self.logging_level = get_or_error(
            log, "level", error_base % "Logging level", logging.getLevelName(log_level)
        )

        self.pid_file = get_or_error(
            service, "pid-file", error_base % "PID file", default=pid_file
        )
        self.save()
    ...
    def type(self, new_type: str):
        ...
        if new_type not in VALID_RECORD_TYPES:
            raise ValueError(
                f"Record type '{new_type}' is not a valid value! "
